...
public class DefaultSqlPrepareStatementStrategy {
    ...
    private static final GeorgeLogger LOG = GeorgeLoggerFactory.getLogger(DefaultSqlPrepareStatementStrategy.class);

    public static void main(String[] args) throws SQLException{
        DefaultSqlPrepareStatementStrategy test = new DefaultSqlPrepareStatementStrategy();
        System.out.println(test.prepareQuery("insert into TABLE (fieldA,fieldB,fieldC) values (:fieldA,':fieldB',:fieldC)",true));
    }
    ...
    public String prepareQuery(String query, boolean allowNamedParameters) throws SQLException {
        ...
        System.out.println("Prepared query: {}"+answer);
        return answer;
    }
                ...
                return new Iterator<Object>() {
                    ...
                    public Object next() {
                        if (next == null) {
                            ...
                            if (!contains) {
                                throw new RuntimeException("Cannot find key [" + key + "] in message body or headers to use when setting named parameter in query [" + query + "]", exchange);
                            }
